...
public class FastPrintWriter extends PrintWriter {
  ...
  private void flushLocked() throws IOException {
    ...
    if (mPos > 0) {
      if (mOutputStream != null) {
        ...
        while (nonEolOff < len && mText[mPos - 1 - nonEolOff] == mSeparator.charAt(
            mSeparator.length() - 1 - nonEolOff)) {
          nonEolOff++;
        }
        if (nonEolOff >= mPos) {
          mPrinter.println("");
  ...
  @Override public void print(int inum) {
    if (inum == 0) {
      print("0");
    } else {
      super.print(inum);
  ...
  @Override public void print(long lnum) {
    if (lnum == 0) {
      print("0");
    } else {
      super.print(lnum);
  ...
  @Override public void println(int inum) {
    if (inum == 0) {
      println("0");
    } else {
      super.println(inum);
  ...
  @Override public void println(long lnum) {
    if (lnum == 0) {
      println("0");
    } else {
      super.println(lnum);
   ...
   * @return this writer.
   * @throws StringIndexOutOfBoundsException if {@code start > end}, {@code start < 0}, {@code end
   * <
   * 0} or
