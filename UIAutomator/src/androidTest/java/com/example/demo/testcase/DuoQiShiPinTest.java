 ...
 * 徒弟阅读  100
 */
public class DuoQiShiPinTest extends BaseTest {

  // 次数统计
  private int readCount = 0; // 阅读次数
  private int checkCount = 0;

  public DuoQiShiPinTest() {
    super();
  }
  ...
  private UiObject2 checkInMainPage() {
    ...
    while (restartCount < 10) {
      ...
      UiObject2 tabs = findById("btn_home_left");// 底部导航栏ID为Text
      if (tabs == null) {// 如果找不到底部导航栏有可能是有对话框在上面
        logE("检查失败,没有[底部导航栏]:" + restartCount);
        closeDialog();
        tabs = findById("btn_home_left");
  ...
  private boolean checkIsPlayVideo() {
    // 是否正在播放
    UiObject2 star = findById("btn_star", 3);
    if (star != null) {
      logD("正在播放界面");
      return true;
    }
    ...
    while (repeatCount++ < 10) {
      ...
      // 检测是否在首页第一个tab
      star = findById("btn_star", 3);
      if (star == null) {// 如果不在主页第一个tab
        UiObject2 homeLeft = findById("btn_home_left");
        if (homeLeft != null) {
          logD("切换到首页,或者刷新");
          homeLeft.click();
        }
      ...
      // 如果向左滑动了
      UiObject2 icon = findById("dotask", 5);
      if (icon != null) {
        pressBack("点击返回,回到首页", false);
        continue;
      }
      // 如果向右滑动了
      icon = findById("tv_fans", 5);
      if (icon != null) {
        pressBack("点击返回,回到首页", false);
        continue;
      }
  ...
  private boolean doPlayVideo() {
    ...
    if (readCount % 12 == 0) {
      UiObject2 comment = findById("badgeView", 3);
      if (comment != null && !"0".equals(comment.getText())) {
        comment = findById("btn_star");
        if (comment != null) {
          comment.click();
          sleep(1);
          mDevice.waitForIdle(timeOut);
    ...
    // 关注
    if (readCount % 2 == 0) {
      UiObject2 guanzhu = findById("iv_guanzhu");
      if (guanzhu != null) {
        guanzhu.click();
        sleep(1);
        mDevice.waitForIdle(timeOut);
